	...
	<view class="page-index" :class="{ bgf: navIndex > 0 }">
		...
		<view class="page_content" :style="'margin-top:' + (marTop + 30) + 'px;'" v-if="navIndex == 0">
			...
			<view class="barg" v-if="bargList.length > 0">
				<view class="title"><image src="/static/images/barg001.png" mode=""></image></view>
					...
					<view class="left">
						...
						<view class="person">
							<view class="avatar-box">
								<block v-for="item in pinkInfo.avatars"><image :src="item" mode=""></image></block>
							</view>
			...
			<view class="index-product-wrapper" style="min-height: 1500rpx;">
				<view class="nav-bd">
					<view class="item" v-for="(item, index) in explosiveMoney" :index="item.id" :class="{ active: index == ProductNavindex }" @click="ProductNavTab(item, index)">
				...
				<view class="mores-txt flex" v-if="!goodScroll"><text>Tôi cũng có một điểm mấu chốt</text></view>
			</view>
		</view>
		<!-- 分类页 -->
...
let app = getApp();
import { getIndexData, getCoupons, pink } from '@/api/api.js';
// #ifdef MP-WEIXIN
...
import { getCategoryList, getProductslist, getProductHot, getGroomList } from '@/api/store.js';

import guide from '@/components/guide/guideBaners';
...
export default {
	computed: mapGetters(['isLogin', 'uid', 'showGuideType']),
	components: {
		tabNav,
		goodList,
		promotionGood,
	...
	onLoad() {
		...
		Promise.all([this.getAllCategory(), this.getIndexConfig(), this.getSeckillIndexTime(), this.getBargainList(), this.getCombinationList(), this.pink(), this.setVisit()]);
		// #ifdef MP
		this.getTemlIds();
		this.getLiveList();
		// #endif
